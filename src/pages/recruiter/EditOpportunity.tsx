...
export default function EditOpportunity() {
  ...
  const onSubmit = async (formData: OpportunityFormData, status?: "draft" | "published") => {
    if (!id) return;
    setSaving(true);
    try {
      // Update opportunity
      ...
      queryClient.invalidateQueries({ queryKey: ["recruiter-opportunity-applicants", id] });
    ...
    <div className="bg-muted/30">
      <div className="container py-8 max-w-4xl">
        ...
        <div className="mb-8">
          ...
          <p className="text-muted-foreground">Update your opportunity — changes will be visible to students immediately.</p>
          {data.opportunity.filled && (
            <Badge className="mt-2 bg-destructive/10 text-destructive">Position Filled — Editing is still allowed</Badge>
          )}
        </div>
          ...
          <form className="space-y-8">
                ...
                <FormField control={form.control} name="skills_required" render={() => (
                  <FormItem>
                    ...
                    <div className="flex gap-2">
                      ...
                      <Input placeholder="Or type custom skill" value={skillInput} onChange={(e) => setSkillInput(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") { e.preventDefault(); addSkill(skillInput); } }} className="flex-1" />
                ...
                <div className="grid sm:grid-cols-2 gap-6">
                  ...
                  <FormField control={form.control} name="duration_hours" render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl><Input type="number" min={10} max={160} {...field} onChange={(e) => field.onChange(parseInt(e.target.value))} /></FormControl>
                      <FormMessage />
                    </FormItem>
                  )} />
                </div>
                ...
                <div className="grid sm:grid-cols-2 gap-6">
                  <FormField control={form.control} name="max_applicants" render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl><Input type="number" min={1} max={100} {...field} onChange={(e) => field.onChange(parseInt(e.target.value))} /></FormControl>
                      <FormMessage />
                    </FormItem>
                  )} />
            ...
            <Card className={form.formState.errors.tasks?.message ? "border-destructive" : ""}>
              <CardHeader>
                ...
                <CardDescription>Define the tasks students will complete</CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                        ...
                        <div className="flex-1 space-y-4">
                          <FormField control={form.control} name={`tasks.${index}.title`} render={({ field }) => (
                            <FormItem>
                              ...
                              <FormControl><Input placeholder="e.g., Build responsive dashboard" {...field} /></FormControl>
                              <FormMessage />
                            </FormItem>
                          )} />
                          <FormField control={form.control} name={`tasks.${index}.description`} render={({ field }) => (
                            <FormItem>
                              <FormLabel>Description</FormLabel>
                              <FormControl><Textarea placeholder="Describe what the student should accomplish..." rows={3} {...field} /></FormControl>
                              <FormMessage />
                            </FormItem>
                          )} />
                          <FormField control={form.control} name={`tasks.${index}.due_days`} render={({ field }) => (
                            <FormItem>
                              <FormLabel>Due (days after acceptance)</FormLabel>
                              <FormControl><Input type="number" min={1} max={30} {...field} onChange={(e) => field.onChange(parseInt(e.target.value))} /></FormControl>
                              <FormMessage />
                            </FormItem>
                          )} />
                        </div>
                ...
                <Button type="button" variant="outline" onClick={() => append({ title: "", description: "", due_days: 7 })} className="w-full gap-2">
            ...
            <div className="flex gap-4 justify-end">
              ...
              <Button type="button" onClick={() => form.handleSubmit((d) => onSubmit(d))() } disabled={saving} className="gap-2">
